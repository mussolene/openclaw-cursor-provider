...
function serializeMessage(msg, opts = {}) {
    ...
    if (msg.role === "assistant") {
        ...
        for (const block of msg.content) {
            ...
            if (block.type === "toolCall") {
                chunks.push(`[assistant tool ${block.name} id=${block.id}]\n${JSON.stringify(block.arguments ?? {}, null, 2)}`);
            }
        }
        return chunks.join("\n\n");
    }
...
function extractSlimSystem(systemPrompt, maxChars) {
...
function orchestrationGuard(lean) {
    const parts = [OPENCLAW_ORCHESTRATION_GUARD.trim(), ANTI_LOOP_RULES.trim()];
    if (lean)
        parts.push(LEAN_TOOL_HINT.trim());
    return parts.join("\n");
}
function followupGuard(lean) {
    const parts = [OPENCLAW_FOLLOWUP_GUARD.trim(), ANTI_LOOP_RULES.trim()];
    if (lean)
        parts.push(LEAN_TOOL_HINT.trim());
    return parts.join("\n");
}
function serializeOptions(config, toolTurn) {
    return { includeThinking: toolTurn && config.includeThinkingInPrompt };
}
function appendSerializedMessages(parts, messages, config, toolTurn) {
...
export function buildSlimCursorPrompt(context, config = DEFAULT_CHAT_MODE_CONFIG) {
    const parts = [OPENCLAW_CHAT_GUARD.trim()];
...
export function buildSlimFollowUpPrompt(context, config = DEFAULT_CHAT_MODE_CONFIG) {
    ...
    for (const msg of reversed) {
        ...
        if (activeToolResultOnly(msg)) {
            const body = partText(msg.content);
            chunks.unshift(`[tool ${msg.toolName} id=${msg.toolCallId}${msg.isError ? " error" : ""}]\n${body}`);
            continue;
        }
        if (msg.role === "assistant") {
            break;
...
export function buildCursorFollowUpPrompt(context, config = DEFAULT_CHAT_MODE_CONFIG) {
    if (!needsTools(context, config)) {
    ...
    if (!chunks.length)
        return buildCursorPrompt(context, config);
