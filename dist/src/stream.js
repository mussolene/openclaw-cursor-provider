...
function buildAssistant(model, params) {
    const content = [];
    if (params.thinking)
...
export function createCursorSdkStreamFn(params) {
    ...
    return (model, context, options) => {
        ...
        void (async () => {
            ...
            const pushPartial = (stopReason = "stop", toolCall) => buildAssistant(billedModel, {
            ...
            const handleToolIntent = (intent) => {
                if (!intent || pendingStop)
                    return;
            ...
            const appendThinkingDelta = (chunk) => {
                if (!chunk)
                    return;
                if (!thinkingStarted) {
            ...
            const onDelta = (update) => {
                ...
                if (update.type === "thinking-delta" && chunk)
                    appendThinkingDelta(chunk);
                if (update.type === "thinking-completed")
                    finalizeThinking();
                if (update.type === "text-delta" && chunk)
            ...
            try {
                ...
                activeRun = await agent.send(prompt, {
                    ...
                    onStep: ({ step }) => {
                        if (step.type === "toolCall" && step.message) {
                            ...
                            if (synthetic)
                                handleToolIntent(parseCursorToolIntent(synthetic, allowedTools));
                        }
                    },
                    local: { customTools },
                });
                ...
                try {
                    for await (const _event of activeRun.stream()) {
                        ...
                        if (pendingStop) {
                            if (turnEnded)
                                break;
                            if (stopDrainDeadline > 0 && Date.now() > stopDrainDeadline)
                                break;
                        }
                    }
                }
                ...
                if (shouldCancelRun) {
                    ...
                    if (strictToolLoop && toolTurn && activeRun.supports("conversation")) {
                        ...
                        if (conversationHadInternalTools(turns)) {
                            pendingStop = {
                                kind: "error",
                                message: "Cursor executed built-in tools internally (Mode A). OpenClaw provider requires tools via harness — retry or use sessions_spawn/ACP for deep file work.",
                                text: accumulatedText,
                                thinking: accumulatedThinking,
                                usage: billedUsage(),
                            };
                ...
                const usage = rawUsage().totalTokens > 0
                    ? billedUsage()
